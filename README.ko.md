...
| 4 | **Deploy** | Model List / Activity (세로 2패널) | 런타임이 아니라 프로비저닝 — 위/아래 2패널(`Ctrl+w` 로 포커스 전환). **Model List**(위): 배포 가능한 모든 것 — 카탈로그 가능성(`✓ ready`/`⚙ needs-compile`/`✗ no-capacity`) + 배치 타깃·스토어 빌드, family 로 묶음; `⏎` → Deploy/Compile. **Activity**(아래): compile Job(진행률 % 바)과 시도 중·실패한 deploy rollout 을 한 피드로; `⏎` → Logs/Delete |
