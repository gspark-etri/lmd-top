...
impl App {
    ...
    // (filter state + per-view search_text moved to src/app/filter.rs)
    ...
    // (column sorting moved to src/app/sort.rs)

    // (snapshot apply + history + alert detection moved to src/app/state.rs)
