...
fn view_overview(f: &mut Frame, area: Rect, app: &App) {
    ...
    for (kind, node, cnt, us, mu, mt, alive, thr, model) in &groups {
        ...
        sp.extend(bar_line(util, 10, util_color(util)).spans); // overview 는 레인보우 바(장식) — 수치는 severity 색으로 의미 유지
        sp.push(Span::styled(format!(" {:>3.0}% ", util), Style::default().fg(util_color(util))));
        sp.push(Span::styled("mem ", Style::default().fg(C_DIM())));
        sp.extend(bar_line(mempct, 10, mem_color(mempct)).spans); // MEM 도 레인보우 바 — 유휴 때도 채움이 보임
...
fn accel_brief(a: &crate::collect::Accel, branch: &str, full: bool) -> Line<'static> {
    ...
    sp.extend(bar_line(a.util, 8, util_color(a.util)).spans);
...
fn view_nodes(f: &mut Frame, area: Rect, app: &App) {
    ...
    for (pos, &i) in order.iter().enumerate() {
        ...
        h.extend(bar_line(if n.cpu_pct.is_nan() { 0.0 } else { n.cpu_pct }, 8, util_color(n.cpu_pct.max(0.0))).spans);
        h.push(Span::styled(
